...
async function main() {
  ...
  cli.command('<prompt>', 'Ask ChatGPT a question').option('-c, --continue', 'Continue last conversation', {
    default: false
    ...
    default: true
  }).option('-t, --timeout <timeout>', 'Timeout in milliseconds').option('-k, --apiKey <apiKey>', 'OpenAI API key').option('-o, --apiOrg <apiOrg>', 'OpenAI API key').option('-m, --model <model>', 'Model (gpt-3.5-turbo, gpt-4)', {
    default: 'gpt-3.5-turbo'
  }).option('-n, --conversationName <conversationName>', 'Unique name for the conversation').action(async (prompt, options) => {
    ...
    const conversation = options.continue && options.store ? config.get(conversationKey, {}) || {} : {};
    ...
    const res = await api.sendMessage(prompt, {
      ...
      onProgress: options.stream ? progress => {
        if (progress.delta) {
